...
- **Status**: Accepted — Opus 4.8 (2026-06-02). 3 待定决策已确认（BoyangJiao）：决策 6 = **加密落盘**；决策 4a = **常显「更新于 HH:MM」+ 断网提示行（两者都加）**，并加 refresh-in-progress 软提示缓解冷启动跳变；决策 3a = **historical 不持久化**。Ready for Sonnet（Opus 先出 serializer 核心）。
...
2. ✅ **DONE（Opus, commit `cb6ccbd`）`feat(mobile): Decimal-safe JSON serializer + tests`** — `apps/mobile/src/lib/cache/decimal-safe-json.ts`：`encodeDecimals`/`decodeDecimals` + `serialize`/`deserialize`（Decimal `{__arc_dec__}` 标记，lossless）。11 测试含真实 PortfolioValuation round-trip（AC.OF.6 满足）。**Sonnet 的 `query-persister.ts` 直接 import 这两个函数作 persister 的 serialize/deserialize，不要重写。**
...
4. **`feat(mobile): encrypt MMKV at rest (secure-store key)`** — 随机 key 写 `expo-secure-store`，MMKV 异步取 key 初始化，并入 rehydrate gate（决策 6）。**先于 wiring 落**，确保从一开始就加密、无明文迁移。
5. **`feat(mobile): "更新于 HH:MM" timestamp + refresh-in-progress + offline row`** — Hero 常显更新时间（决策 4a 跳变缓解核心）+ 后台刷新软提示 + `onlineManager`/NetInfo 断网行；i18n en+zh；遵守免责文案体系。
6. **`feat(mobile): cache lifecycle on logout / env switch`** — 登出/Real↔Clean 切换清缓存（决策 9，复用现有 reset 路径，防串号）。
7. **`docs(spec+session-state): offline cache complete`**。

...
- **Status**: Draft — Opus 4.8 (2026-06-02); awaiting BoyangJiao review
...
**决策 3a 待确认**：是否持久化 `historical`（详情页历史曲线）？建议**否**（数据量大、按标的+时间窗爆炸、详情页可接受短暂加载）。

**决策 4 — stale 显示策略：先显示缓存 + 后台刷新（stale-while-revalidate）**
rehydrate 后 query 立即以 stale 数据渲染，`staleTime`（现 60s）到期触发后台 refetch。**法则 5 红线**：缓存是「上次看到的」，UI 不可把它当「当前实时」误导用户。**决策 4a 待确认**：是否在 Hero/持仓加一个轻量「数据可能延迟 · 上次更新 HH:MM」标识？建议**复用现有「仅供参考，可能延迟」免责文案体系**，缓存态不额外加显眼 badge（避免噪音）；或仅在**断网**时显示一行 offline 提示。
...
**决策 6 — 敏感金额是否加密落盘**
缓存含真实持仓金额，落 MMKV 明文。`react-native-mmkv` 支持 `encryptionKey`。**决策 6 待确认**：是否加密？建议 **加密**（金额是隐私，与脱敏特性同源诉求），key 存 `expo-secure-store`（Keychain/Keystore）。代价：MMKV 实例需异步取 key 后初始化 → rehydrate gate 要等 key。若嫌复杂，**MVP 可先不加密**（设备本身有锁屏 + 沙盒），加密留 follow-up —— 但**须在 spec 记为已知风险**。
...
| **S3-AC-OF.5**  | 白名单生效：`symbol-search` / `historical`(若决策 3a=否) 不被持久化（不污染磁盘 / 不显示陈旧搜索）                 |
...
| **S3-AC-OF.8**  | （决策 6=加密时）MMKV 文件落盘加密，明文不可读；key 在 secure-store                                                |
...
1. **`chore(mobile): add react-native-mmkv + query-persist-client`** — `npx expo install` 对齐 SDK 55；确认 iOS/web build（web 无 MMKV → 降级 noop persister 或 IndexedDB，留注释）。
2. **`feat(mobile): Decimal-safe query persister (MMKV) + tests`** — `query-persister.ts`：MMKV storage adapter + 递归 serialize/deserialize（Decimal `{__dec}` 标记）。**重单测**（AC.6）：嵌套 Decimal / 数组 / null / 非 Decimal 混合 round-trip。**纯函数核心可单测，不碰 RN。**
3. **`feat(mobile): persist gate + whitelist dehydrate + gcTime`** — `_layout.tsx` 接 `PersistQueryClientProvider`（buster + maxAge 24h + `shouldDehydrateQuery` 白名单，决策 3）；`query-client.ts` 给白名单 query 设 `gcTime ≥ maxAge`；rehydrate gate（决策 7）。
4. **`feat(mobile): offline indicator + cache lifecycle`**（可选 per 决策 4a）— 断网提示行 + 登出/环境切换清缓存（决策 9，复用 reset 路径）。
5. **`feat(mobile): encrypt MMKV at rest`**（若决策 6=加密）— secure-store key + 异步初始化 gate。
6. **`docs(spec+session-state): offline cache complete`**。
...
- **决策 3a**（historical 是否持久化）、**决策 4a**（stale/offline 提示）、**决策 6**（加密 vs MVP 明文）— 待 BoyangJiao 确认。

...
>
> **Last updated**: 2026-06-01 by Opus 4.8（续）— **CSV 导入 spec = Accepted**（含可扩展格式 profile 架构）。BoyangJiao 同意决策 1a（忽略 CSV portfolio_id，导入到 UI 选定组合）+ 4a（导入跳 US 在线校验，避免 Finnhub 限流），并提出**导入需兼顾各券商/平台格式**。spec 据此升级决策 1 为**三层解耦**：L1 RFC4180 raw（格式无关）/ L2 `ImportProfile`（格式特定，本轮仅 `arc-native`）/ L3 validator+writer（canonical 行，加新平台零改）。本轮只交付 arc-native profile，架构备好 seam；后续每个平台单独一轮、按真实 CSV 模板加 1 个 profile + 单测。commit 链调整为 6 步（拆出 2b profile registry），AC 加 FI.12（profile 架构）。**可交接 Sonnet 实现**（`csv-import-stage-3.md` 交接 prompt 已更新）。
>

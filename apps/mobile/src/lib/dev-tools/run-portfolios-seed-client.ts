...
import { isBlockCPortfolioScenario, type PortfolioScenarioId } from "./scenarios";
import {
  PortfoliosBlockCSeedError,
  runPortfoliosBlockCSeed,
} from "../../../../../supabase/functions/_shared/seed-portfolios-block-c";
...
export const runPortfoliosSeedClient = async (
  ...
  if (isBlockCPortfolioScenario(scenario)) {
    try {
      return await runPortfoliosBlockCSeed(supabase, userId, scenario);
    } catch (err) {
      const msg =
        err instanceof PortfoliosBlockCSeedError
          ...
          : err instanceof Error
            ? err.message
            : String(err);
      throw new PortfoliosSeedError(msg);
    }
  }

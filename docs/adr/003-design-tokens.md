...
#### 双命名空间：Foundation `--X` vs Tailwind `--color-X`

HeroUI Native 主题存在**两条并行颜色通道**（见 `heroui-native/lib/module/styles/theme.css`）：

| 通道              | 变量前缀                            | 谁消费                                         | Arc 在哪改                                                |
| :---------------- | :---------------------------------- | :--------------------------------------------- | :-------------------------------------------------------- |
| **Foundation**    | `--accent-soft-foreground` 等       | `useThemeColor()`、手写 `var(--*)`             | `global.css` `@layer theme`（Theme Builder / light·dark） |
| **Tailwind 工具** | `--color-accent-soft-foreground` 等 | `text-*` / `bg-*` className（HeroUI 组件样式） | HeroUI 包内 `@theme inline static` 默认公式               |

多数 token 两通道一致（`--color-accent: var(--accent)`）。**例外**：Calculated Variables 段把四个 `*-soft-foreground` 硬绑为 `var(--accent|success|danger|warning)`，不读 Foundation 的 `--*-soft-foreground`。Arc light 模式将 `--accent-soft-foreground` 设为 eclipse 时，仅改 Foundation 不会让 `text-accent-soft-foreground` 生效。

**维护规则**：凡在 `@layer theme` 自定义的 Foundation 与 HeroUI 默认 `--color-*` 公式不一致时，在 `apps/mobile/global.css` 的 `@theme inline` 块追加桥接：

```css
--color-accent-soft-foreground: var(--accent-soft-foreground);
/* 同族 success / danger / warning 一并桥接，见 global.css */
```

Arc-only token（`--info`、`--skeleton`）若需 className 消费，同样在此注册 `--color-info: var(--info)` 等。完整清单见 `packages/ui/DESIGN-TOKENS.md` §Tailwind 桥接清单。

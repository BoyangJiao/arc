...
**Supabase 默认行为重新评估**（已通过 Dashboard 实测，2026-05-16 修正）：

- access_token 默认 1h，自动通过 refresh token 续
- **Refresh token 在 Free tier 没有 absolute lifetime** —— 默认事实上"永不过期"，除非：
  - 用户主动 `signOut()`
  - Dashboard 手动 revoke
  - "Detect and revoke potentially compromised tokens" 检测到异常（多端反复 refresh 等）触发自动吊销
- "Time-box user sessions" / "Inactivity timeout" 是 **Pro Plan 专属**功能（Free tier Dashboard 不显示可编辑控件）
...
也就是说"每次都要收验证码"这个痛点的真实根因不是 Supabase 太严格，**是 DEV_BYPASS_AUTH 让真实持久化路径根本没被走过**。Free tier 默认行为已经是"登录一次基本顶到 dev 期结束"。
...
### 决策二：dev 期间登录持久化完全靠 Supabase Free tier 默认行为（零配置）

工作流：

1. 第一次启动 dev 环境，用真实邮箱走 OTP 流程登录（30 秒）
2. 之后冷启动 / Metro 重启 / 切分支 / 重启 Expo Go：**SDK 从 AsyncStorage 恢复 session，直接进 app，不需要重新输验证码**
3. Free tier refresh token 没有 absolute lifetime，**事实上永不过期**直到用户主动 signOut 或异常检测触发吊销

**Supabase dev project 配置**（保持默认即可）：

- Authentication → Sessions → Refresh Tokens → "Detect and revoke potentially compromised refresh tokens": **保持 ON**（默认；防 replay 攻击）
- Authentication → Sessions → Refresh Tokens → `Refresh Token Reuse Interval`: **保持默认 10s**
- Authentication → Sessions → User Sessions → "Time-box user sessions" / "Inactivity timeout": **Pro Plan 专属**，Free tier 不可配置——自然行为已经是"无超时"

**早期 ADR 草稿错误**：本节早先版本写"调到 60 天"，是基于 Pro 行为的误解。Free tier Dashboard 实际**没有这一项**，且免费默认行为（无 absolute lifetime）已经达到"登录一次顶到 dev 期结束"的目标，**不需要任何手动操作**。Prod 上若未来升 Pro 再来评估 time-box 策略。
...
1. **配置不依赖本机 keychain / macOS-only 路径** —— service role key 读取自 `.env.dev.local (repo root)`（gitignored），同时提供 `.env.dev.example (repo root)` 作为模板入仓，写明每个变量从 Supabase Dashboard 的哪条路径取
2. **配置过程文档化** —— 在 [docs/development-plan.md](../development-plan.md) 末增「Dev 环境自举（任意机器/任意时刻）」章节，按 8 步可复现：
   ...
   4. `cp .env.dev.example (repo root) .env.dev.local (repo root)` + 填 service role key
...
1. [x] ~~**Supabase dev project 配置**：Dashboard 把 refresh token absolute lifetime 调到 60 天~~ — Free tier 无此选项；默认行为已达成目标，**跳过**（2026-05-16 修正）
2. [ ] **删 DEV_BYPASS 链路**：
...
4. [ ] **建 seed 脚本** `tools/seed-dev-data.ts`：
   ...
   - 配套 `.env.dev.example (repo root)`（模板入仓）+ `.env.dev.local (repo root)`（gitignored）

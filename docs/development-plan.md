...
| **美股 / ETF**           | ~~Alpha Vantage~~ **Finnhub**（60/min free）¹；CN/HK 仍 Tushare Stage 3 P0 | 免费起步               | 充足 MVP       | ✅ US 已换 Finnhub（2026-05-19） |     P0     |
| **加密货币**             | CoinGecko 公共 API                                                         | 免费                   | 30 calls/min   | ✅                               |     P0     |
| **汇率**                 | exchangerate.host（免费）/ OpenExchangeRates（$12/月）                     | 免费起步               | 当日 + 历史    | ✅                               |     P0     |

¹ **2026-05-19**：默认 US 行情已从 Alpha Vantage 切到 Finnhub（`feat/finnhub-adapter`）；`createAlphaVantageAdapter` 保留回滚。A 股 / 港股 / 加密 / 基金仍属 Stage 3 P0 清单。

...
- 点击：从左侧滑入**全屏页面**（根 Stack push，`animation: 'slide_from_left'`），非抽屉
- 返回：左上角 X / 系统返回键 / **右缘向左滑**关闭整个 `/me`（与从左推入对称；`animationMatchesGesture` + `fullScreenGestureEnabled`）
- 子页（如 `/me/settings`）：在 `app/me/_layout.tsx` **子栈**内以默认 iOS 自**右**推入，手势为左缘向右返回上一层
...
│
└─ /me                                         ← Me 全屏页（顶栏头像点击触发；根栈 slide_from_left）
   └─ → /me/settings                           ← 设置（me 子栈内自右 push，与根进入方向不同）
```
...
| `/me`                              | `app/me/_layout.tsx` + `app/me/index.tsx` | Nested Stack（根） | Me 全屏页；根栈 `slide_from_left`，右缘左滑关闭 |
| `/me/settings`                     | `app/me/settings.tsx`                     | Nested Stack（子） | 设置；子栈默认自右 push                         |

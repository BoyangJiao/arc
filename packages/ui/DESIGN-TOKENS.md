...
2. 若 Foundation 与 HeroUI 默认 `--color-*` 公式不一致 → 在 `global.css` `@theme inline` 桥接（见下方 §Tailwind 桥接清单）。
3. 若动到 `--background` / `--surface` / `--border` / `--muted` / `--accent` → 同步 [`navigation-colors.ts`](src/tokens/navigation-colors.ts) 注释中的 hex。
4. 更新本文件 YAML frontmatter + 下方主表对应行（至少：accent、background、muted、surface 三层、accent-soft 派生）。
5. `pnpm typecheck` + 真机 light/dark 各扫一眼 TabBar、主按钮、ToggleButton 选中、Button secondary。
...
HeroUI `theme.css` 的 className 工具色走 `--color-*`，不自动读 `@layer theme` 的 Foundation `--*`。下列项在 `apps/mobile/global.css` 显式桥接；**新增 Arc Foundation 且要用 className 消费时**，按同模式追加一行。

| Tailwind 工具变量                   | 桥接到 Foundation                | 状态                                                  |
| :---------------------------------- | :------------------------------- | :---------------------------------------------------- |
...
| `--color-info` / `--color-skeleton` | —                                | 未桥接（仅 StyleSheet / `navigation-colors.ts` 消费） |

**接新 HeroUI 组件时 30 秒检查**：grep 组件 `*.styles.*` 里的 `text-*` / `bg-*` → 对照 `heroui-native/.../theme.css` 该 token 是 `var(--foundation)` 还是 Calculated 公式 → 若与 Arc Foundation 分叉则加上表一行。

**本文档与 `global.css` 同步日期**：2026-05-22（soft-foreground 整族 Tailwind 桥接 + light accent-soft 标签 eclipse 修复）。
...
| `accent-soft-foreground`       | `#18181B`                  | `#50FF6C`                  | `Light: eclipse; Dark: accent` — **must** bridge (see §Tailwind 桥接清单; HeroUI default `--color-*` → `var(--accent)`). | `--accent-soft-foreground`       | `text-accent-soft-foreground` on ToggleButton selected / Button `secondary` label | Text on accent-soft surfaces; not the same as `text-accent`.                        |
...
| `danger-soft-foreground`       | `#FD3367`                  | `#FD3367`                  | `= danger` (bridge `--color-danger-soft-foreground` in global.css)                                                       | `--danger-soft-foreground`       | `text-danger-soft-foreground` on danger-soft badges                               | Text color on danger-soft backgrounds.                                              |
...
| `success-soft-foreground`      | `#64C33A`                  | `#64C33A`                  | `= success` (bridge `--color-success-soft-foreground` in global.css)                                                     | `--success-soft-foreground`      | `text-success-soft-foreground` on success-soft badges                             | Text color on success-soft backgrounds.                                             |
...
| `warning-soft-foreground`      | `#FF9C3E`                  | `#FF9C3E`                  | `= warning` (bridge `--color-warning-soft-foreground` in global.css)                                                     | `--warning-soft-foreground`      | `text-warning-soft-foreground` on warning-soft badges                             | Text color on warning-soft backgrounds.                                             |
